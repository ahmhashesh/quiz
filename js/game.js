...
function setupTouchEvent(){
	...
	touchAreaContainer.addEventListener("click", handlerTouchMethod);
}

function removeTouchEvent(){
	touchAreaContainer.cursor = null;
	touchAreaContainer.removeEventListener("click", handlerTouchMethod);
}
...
function playAudioLoop(){
	...
	if(audio_arr[audioNum].type == 'question'){
		playAudio(audio_arr[audioNum].id, playAudioComplete);
...
function updateGame(){
	if(gameTimerUpdate){
		nowDate = new Date();
		currentTime = (nowDate.getTime() - beforeDate.getTime()) + accumulateTimer;
		
		playerData.timerCount = ((currentTime/1000)%60);
		updateTimer(true);
